<script>
  ...
  const actionClass = `inline-flex min-h-11 items-center justify-center border border-[var(--line-soft)]
    bg-ink-2 px-3 py-2.5 font-mono text-[11px] tracking-[0.04em] text-paper
    outline outline-2 outline-transparent outline-offset-2 transition-[border-color,color,transform] duration-150
...
<fieldset class="grid min-w-0 gap-2.5 border-0 p-0" {disabled}>
  ...
  {#if url && !previewFailed}
    <div class="aspect-video w-[min(100%,380px)] overflow-hidden border border-[var(--line-soft)] bg-ink-2">
      <img
